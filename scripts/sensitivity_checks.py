...
Checks included:
1) Leave-out sensitivity excluding rescue families (10 families in the released dataset; 12 rescued records before deduplication)
2) CPC bridging sensitivity using classified-only families (excluding 80 domain-external families)
3) Jurisdiction-stratified sensitivity: CN vs non-CN subsets (Supplementary Table S2)
4) Shared-anchor inflation check: 4-1 ↔ 4-4 Jaccard recomputation excluding E04H15-only families
...
def main():
    ...
    md.append("")
    md.append(f"- Domain-external families excluded in the classified-only run: **{len(families) - len(classified)}**")
    md.append(f"- Leading bridging trio unchanged (B64G, E04H, G01N): **{'Yes' if trio_same else 'No'}**")
    md.append("")
    md.append("Top 10 CPC bridging codes — all 453 families")
    md.append("")
    md.append(markdown_table(
    ...
    md.append("")
    md.append(f"- CN families: **{len(cn_fams)}** ({len(cn_fams)/len(families)*100:.1f}%)")
    md.append(f"- Non-CN families: **{len(non_cn_fams)}** ({len(non_cn_fams)/len(families)*100:.1f}%)")
    md.append("")
